...
int close_key = VK_F6, toggle_key = VK_F7; // Keystrokes

/* AUTOCLICKER */

void *autoclicker(void *args) {
  while (running) {
...
void *RunClickerA(void *args) {
  ...
  delay = (1000 / CPS) - (CPS/2);

  /* GRAPHICS */

  SetConsoleTitleA("TS Clicker 1.0.0b");
  ...
  pthread_create(&clickth, NULL, autoclicker, NULL); // Creates a clicker thread
...
void print_home() {
  ...
  printf("                                          |\n");
  printf("\n\n\n");
  printf("                                 PRESS RIGHT CTRL TO CHANGE SETTINGS\n");
  printf("                           F6 TO CLOSE PERMANENTLY | F7 TO TOGGLE ON/OFF\n\n");
  printf((toggle ? "                                  STATUS: ON (Only works on minecraft)\n" : "                                  STATUS: OFF\n"));
  printf("\n\n\n\n");
}
...
void change_settings() {
  ...
  printf("\n");
  printf("SETTINGS LOADED SUCCESFULLY, RETURNING TO CLICKER IN 1 SECOND ....");
  Sleep(1000);
		...
		case PLUGIN_MENU_TYPE_GLOBAL:
			...
			switch(menuItemID) {
				case MENU_ID_GLOBAL_1:
				    if (!clicker_hidden && can_exec) {

                pthread_create(&main_t, NULL, RunClickerA, NULL);
                can_exec = false;
